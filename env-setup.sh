...
export SNS_REGION=$(env -u ANSIBLE_CALLBACK_PLUGINS ANSIBLE_STDOUT_CALLBACK=json ansible --playbook-dir $hacking_dir/ansible localhost -m debug -a 'msg={{ aws_layout[aws_status][aws_profile_sns].region }}' | jq -r .plays[0].tasks[0].hosts.localhost.msg)
